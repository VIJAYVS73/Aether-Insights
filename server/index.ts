...
app.post('/api/geo/demand-surge-alerts', express.json(), async (req: Request, res: Response) => {
  try {
    ...
    const demandContext = {
      ...
      market_condition: ['bullish', 'strong', 'explosive'][Math.floor(Math.random() * 3)]
    };
    ...
    const aiAlert = {
      ...
      alert_message: `📍 ${city}, ${region} is experiencing a ${demandContext.surge_percentage}% demand surge! Top products: ${productNames || 'Multiple categories'} are trending heavily. ${demandContext.market_condition === 'explosive' ? '🔥 CRITICAL OPPORTUNITY - Act fast!' : demandContext.market_condition === 'strong' ? '⚡ High-opportunity market' : '📈 Growing demand'}`,
      recommendations: [
      ...
      market_insight: `The ${city} market is showing strong purchasing power with focus on ${productList[0]?.category || 'electronics'} category. Regional preferences indicate ${demandContext.market_condition} market conditions.`,
...
app.post('/api/ai/forecast', express.json(), async (req: Request, res: Response) => {
  try {
    ...
    const avgRating = historicalData.length > 0 
      ? (historicalData.reduce((sum: number, d: any) => sum + (d.rating || 4.5), 0) / historicalData.length).toFixed(2)
      : '4.5';
...
app.post('/api/ai/competitive-analysis', express.json(), async (req: Request, res: Response) => {
  try {
    ...
    const analysis = {
      ...
      analysis: {
        strengths: [
          'Superior product quality (4.7★ vs 4.2★ competitor avg)',
          'Competitive pricing strategy',
          'Strong customer retention (68% repeat rate)',
          'Excellent post-sales support'
        ],
        weaknesses: [
          'Limited SKU diversity',
          'Geographic coverage gaps',
          'Brand awareness lower in tier-2 cities'
        ],
        opportunities: [
          'Expansion into underserved segments',
          'Bundle offerings with complementary products',
  ...
  try {
    ...
    const insights = {
      ...
      insights: {
        ...
        growth_potential: 'High (+18% projected)',
        risk_score: 3.2,
        recommendations: [
          'Increase inventory for top 3 SKUs',
          'Launch promotional campaign for slow-moving items',
          'Expand merchant network in high-demand regions'
        ]
      }
    };
